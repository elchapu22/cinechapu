...
// 🔥 NUEVA FUNCIÓN: Obtiene todas las películas de una saga/colección de TMDB
async function obtenerPeliculasDeSaga(collectionId) {
  if (!collectionId) return [];
  const url = `https://api.themoviedb.org/3/collection/${collectionId}?language=es-MX`;
  const options = { method: 'GET', headers: { accept: 'application/json', Authorization: `Bearer ${TMDB_TOKEN.trim()}` } };
  try {
    const res = await fetch(url, options);
    if (!res.ok) return [];
...
bot.on('text', async (ctx) => {
  ...
  if (esperaManual.has(chatId) && /^\d+$/.test(entrada)) {
    ...
    try {
      ...
      if (results.length > 0) {
        // 🔥 TRUCO CLAVE: Con el ID del buscador, pedimos los detalles completos para asegurarnos de traer la SAGA
        peli = await buscarPorIdEnTmdb(results[0].id);
      }
    } catch (err) {}
  }
...
// 🔥 DETECTAR SAGA USANDO EL TEXTO EXACTO DE TURSO
  let sugerencias = [];
  let nombreSagaInfo = "";

  if (peli.belongs_to_collection && peli.belongs_to_collection.name) {
    nombreSagaInfo = ` 📦 (Saga: ${peli.belongs_to_collection.name})`;

    // Limpiamos el nombre de la colección de TMDB (ej: "Mad Max - Colección" -> "mad max")
    const nombreSagaLimpio = peli.belongs_to_collection.name
      .toLowerCase()
      .replace(/(- colección|- collection)/g, '')
      .trim();

    console.log(`📦 Buscando saga en Turso por texto: "${nombreSagaLimpio}"`);

    // Consultamos directo a Turso buscando en el campo id_saga que sabemos que es texto
    const resSaga = await db.execute({
      sql: `SELECT id, nombre, id_saga FROM peliculas WHERE LOWER(id_saga) LIKE ? LIMIT 10;`,
      args: [`%${nombreSagaLimpio}%`]
    });

    sugerencias = resSaga.rows;
    console.log(`📥 Películas encontradas en Turso para esta saga:`, sugerencias.length);
  }

  // Si por alguna razón no encontró nada por saga, buscamos por el título individual de la película
  if (sugerencias.length === 0) {
    const tituloBusquedaSql = `%${peli.title.replace(/[:.,]/g, '').trim()}%`;
    const { rows } = await db.execute({
      sql: `SELECT id, nombre, id_saga FROM peliculas WHERE LOWER(nombre) LIKE LOWER(?) LIMIT 5`,
      args: [tituloBusquedaSql]
    });
    sugerencias = rows;
  }

  // Si sigue sin haber nada, tiramos el respaldo de las últimas añadidas
  if (sugerencias.length === 0) {
    const resBackup = await db.execute("SELECT id, nombre, id_saga FROM peliculas ORDER BY id DESC LIMIT 5");
...
bot.launch();
console.log("🤖 Bot interactivo con detección de Sagas corriendo...");